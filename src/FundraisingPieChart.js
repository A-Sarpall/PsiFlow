...
const FundraisingPieChart = () => {
  ...
  const handleAddAmount = (category) => {
    ...
    const newTotalRaised =
      newDonations + newSponsorships + newEvents + newMerchandise;
  ...
  return (
    <div>
      ...
      <div style={{ display: "inline-flex" }}>
        ...
        <div>
          <h1 style={{ color: "red" }}>Goal Amount: ${goalAmount}</h1>
          <div>
            <h3>
          ...
          <div>
            {totalRaised >= goalAmount && (
              <h2 className="animated-text">Goal Reached!</h2>
            )}
          </div>
        </div>
      </div>
      <div style={{ marginTop: "20px" }}>
        ...
        <button
          className="cat"
          onClick={() => handleAddAmount("Donations")}
          disabled={totalRaised >= goalAmount}
        >
          Add to Donations
        </button>
        <button
          className="cat"
          onClick={() => handleAddAmount("Sponsorships")}
          disabled={totalRaised >= goalAmount}
        >
          Add to Sponsorships
        </button>
        <button
          className="cat"
          onClick={() => handleAddAmount("Events")}
          disabled={totalRaised >= goalAmount}
        >
          Add to Events
        </button>
        <button
          className="cat"
          onClick={() => handleAddAmount("Merchandise")}
          disabled={totalRaised >= goalAmount}
        >
          Add to Merchandise
        </button>
